...
feature 'places', %q{
  ...
  scenario 'view price lists by catalog', js: true do
    c1 = build(:catalog)
    pl = create(:price_list)
    c1.price_lists<<pl
    c1.save!
    c2 = create(:catalog)
    page_login
    ...
    within(:xpath, "//table//tbody//tr[2]//td[2]") do
      page.should_not have_content(I18n.t('views.estimates.catalogs.view_price_lists'))
    end
    within(:xpath, "//table//tbody//tr[1]//td[2]") do
      page.find("span[@class='cell-link']").text.
          should eq(I18n.t('views.estimates.catalogs.view_price_lists'))
      page.find("span[@class='cell-link']").click
    end
